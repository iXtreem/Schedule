...
function resolveWeekId($conn): int
{
    ...
    $weeksSoft = hasColumn($conn, 'TB_Weeks', 'IsDeleted') ? ' WHERE IsDeleted = 0' : '';
    $sql = 'SELECT idWeek AS id FROM TB_Weeks' . $weeksSoft . ' ORDER BY StartDate DESC LIMIT 1';

    try {
        $row = dbRow($conn, $sql);
    } catch (Throwable $e) {
        throw new RuntimeException('WEEK_QUERY_FAILED: ' . $e->getMessage());
    }
    $id = (int)($row['id'] ?? $row['ID'] ?? 0);
    if ($id <= 0) {
        throw new RuntimeException('WEEK_NOT_FOUND');
...
function fetchAllGroupIds($conn): array
{
    ...
    try {
        ...
        throw new RuntimeException('ALL_GROUPS_QUERY_FAILED: ' . $e->getMessage());
    }
...
function fetchGroupCatalog($conn): array
{
    ...
    try {
        ...
        throw new RuntimeException('GROUP_CATALOG_QUERY_FAILED: ' . $e->getMessage());
    }
...
function fetchWeekStartDate($conn, int $weekId): DateTimeImmutable
{
    $weeksSoft = hasColumn($conn, 'TB_Weeks', 'IsDeleted') ? ' AND IsDeleted = 0' : '';
    $sql = "
      SELECT DATE_FORMAT(StartDate, '%Y-%m-%d') AS start_date
      FROM TB_Weeks
      WHERE idWeek = ?{$weeksSoft}
    ";

    try {
        $row = dbRow($conn, $sql, [(int)$weekId]);
    } catch (Throwable $e) {
        throw new RuntimeException('WEEK_QUERY_FAILED: ' . $e->getMessage());
    }
    $start = normalizeWhitespace((string)rowValue((array)$row, 'start_date', ''));
...
function fetchGroupsByIds($conn, array $groupIds): array
{
    ...
    try {
        $rows = dbAll($conn, $sql, array_map('intval', $groupIds));
    } catch (Throwable $e) {
        throw new RuntimeException('GROUPS_QUERY_FAILED: ' . $e->getMessage());
    }
...
function fetchScheduleMatrix($conn, int $weekId, array $groupIds): array
{
    ...
    $params = array_merge([(int)$weekId], array_map('intval', $groupIds));
    try {
        $rows = dbAll($conn, $sql, $params);
    } catch (Throwable $e) {
        throw new RuntimeException('LESSONS_QUERY_FAILED: ' . $e->getMessage());
    }
