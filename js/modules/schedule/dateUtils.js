...
// Границы текущей недели: понедельник (weekStart) и конец недели.
// ВАЖНО (баг «27.12 не берётся»): state.weekEnd заполняется в app.js из
// end_date БД, а SQL-условия вида «date < end_date» обычно хранят end_date
// как ПЕРВЫЙ ДЕНЬ следующей недели (исключительная граница). Напр. неделя
// 21.12.2026–27.12.2026 с end_date = 28.12 должна включать 27-е число, но
// сравнение «t <= end» при end = полночь 28.12 отбрасывало воскресенье 27.12
// из-за времени/часового пояса, и программа отображала неделю некорректно.
// Поэтому здесь end приводится к КОНЦУ суток (23:59:59.999) — числа недели
// берутся строго включительно. Если бэкенд end_date не вернул — считаем
// концом недели сутки «понедельник + 6 дней».
...
export function getWeekBounds() {
  ...
  // Начало суток понедельника — чтобы сравнение «t >= start» было включительным.
  const startDay = new Date(start.getFullYear(), start.getMonth(), start.getDate());
  const rawEnd =
    state.weekEnd && Number.isFinite(new Date(state.weekEnd).getTime())
      ? new Date(state.weekEnd)
      : new Date(startDay.getTime() + 6 * 86400000);
  // Конец недели не дальше воскресенья («понедельник + 6 дней»), но границы
  // включительные: end_date = 27.12 => 27.12 входит в неделю целиком.
  const nominalEnd = new Date(startDay.getTime() + 6 * 86400000);
  const end = toEndOfDay(rawEnd <= nominalEnd ? rawEnd : nominalEnd);
  return { start: startDay, end };
}

// Принадлежит ли дата текущей неделе (от понедельника weekStart до
// конца недели weekEnd ВКЛЮЧИТЕЛЬНО, до последней секунды дня). Нужно
// потому, что таблица перебирает 7 позиций от даты начала недели; когда
// неделя начинается не с понедельника (например, срезанная первая/последняя
// неделя семестра), после воскресенья в переборе идёт понедельник СЛЕДУЮЩЕЙ
// недели — его в таблицу не добавляем, т.к. это уже другая неделя.
// Дата вне диапазона -> false -> строка скрывается.
